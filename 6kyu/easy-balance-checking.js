...
function balance(book) {
  ...
  const arrOfArr = pureArr.map((el) => el.split(" "));
  const startBalance = +pureArr[0];
  let currentBalance = startBalance;
  let average = 0;
  let count = 0;
  
  return arrOfArr.reduce((acc, el, ind, arr) => {
    if (ind === 0) {
      return `Original Balance: ${startBalance.toFixed(2)}\r\n`;
    }
    currentBalance -= +el[2];
    average += +el[2];
    count++;

    const transactionDetails = `${el[0]} ${el[1]} ${(+el[2]).toFixed(2)} Balance ${currentBalance.toFixed(2)}`;
    
    if (ind !== arrOfArr.length - 1) {
      return acc + transactionDetails + "\r\n";
    } else {
      const totalExpense = (startBalance - currentBalance).toFixed(2);
      const avgExpense = (average / count).toFixed(2);
